...
export class ObsidianEvent extends pb_1.Message {
	...
	serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
		...
		if (this.has_create)
			writer.writeMessage(2, this.create, () =>
				this.create.serialize(writer)
			);
		if (this.has_delete)
			writer.writeMessage(3, this.delete, () =>
				this.delete.serialize(writer)
			);
		if (this.has_rename)
			writer.writeMessage(4, this.rename, () =>
				this.rename.serialize(writer)
			);
		if (this.has_modify)
			writer.writeMessage(5, this.modify, () =>
				this.modify.serialize(writer)
			);
		if (this.has_fileOpen)
			writer.writeMessage(6, this.fileOpen, () =>
				this.fileOpen.serialize(writer)
			);
		...
		while (reader.nextField()) {
			...
			switch (reader.getFieldNumber()) {
				...
				case 2:
					reader.readMessage(
						message.create,
						() => (message.create = CreateEvent.deserialize(reader))
					);
					break;
				case 3:
					reader.readMessage(
						message.delete,
						() => (message.delete = DeleteEvent.deserialize(reader))
					);
					break;
				case 4:
					reader.readMessage(
						message.rename,
						() => (message.rename = RenameEvent.deserialize(reader))
					);
					break;
				case 5:
					reader.readMessage(
						message.modify,
						() => (message.modify = ModifyEvent.deserialize(reader))
					);
					break;
				case 6:
					reader.readMessage(
						...
						() =>
							(message.fileOpen =
								FileOpenEvent.deserialize(reader))
					);
					break;
				default:
					reader.skipField();
...
export class CreateEvent extends pb_1.Message {
	...
	static fromObject(data: { filePath?: string }): CreateEvent {
	...
	serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
		const writer = w || new pb_1.BinaryWriter();
		if (this.filePath.length) writer.writeString(1, this.filePath);
		if (!w) return writer.getResultBuffer();
	}
...
export class DeleteEvent extends pb_1.Message {
	...
	static fromObject(data: { filePath?: string }): DeleteEvent {
...
export class RenameEvent extends pb_1.Message {
	...
	serialize(w?: pb_1.BinaryWriter): Uint8Array | void {
		const writer = w || new pb_1.BinaryWriter();
		if (this.newPath.length) writer.writeString(1, this.newPath);
		if (this.oldPath.length) writer.writeString(2, this.oldPath);
		if (!w) return writer.getResultBuffer();
	}
...
export class ModifyEvent extends pb_1.Message {
	...
	static fromObject(data: { filePath?: string }): ModifyEvent {
...
export class FileOpenEvent extends pb_1.Message {
	...
	static fromObject(data: { filePath?: string }): FileOpenEvent {
...
export abstract class UnimplementedObsidianEventStreamService {
	static definition = {
		streamEvents: {
			...
			requestSerialize: (message: ObsidianEvent) =>
				Buffer.from(message.serialize()),
			requestDeserialize: (bytes: Buffer) =>
				ObsidianEvent.deserialize(new Uint8Array(bytes)),
			responseSerialize: (message: Empty) =>
				Buffer.from(message.serialize()),
			responseDeserialize: (bytes: Buffer) =>
				Empty.deserialize(new Uint8Array(bytes)),
		},
	};
	[method: string]: grpc_1.UntypedHandleCall;
	abstract streamEvents(
		call: grpc_1.ServerReadableStream<ObsidianEvent, Empty>,
		callback: grpc_1.sendUnaryData<Empty>
	): void;
}
export class ObsidianEventStreamClient extends grpc_1.makeGenericClientConstructor(
	...
	streamEvents: GrpWritableServiceInterface<ObsidianEvent, Empty> = (
		metadata:
			| grpc_1.Metadata
			| grpc_1.CallOptions
			| grpc_1.requestCallback<Empty>,
		options?: grpc_1.CallOptions | grpc_1.requestCallback<Empty>,
		callback?: grpc_1.requestCallback<Empty>
	): grpc_1.ClientWritableStream<ObsidianEvent> => {
